...
Canonical exit-criteria detail lives in the per-milestone sections below; git log + the code are the authoritative state. Caveats and load-bearing decisions land in `POST_WORK_FINDINGS.md`.

- **Docs scaffolding (2026-05-08).** PRINCIPLES, PLAN, TODO, POST_WORK_FINDINGS, .gitignore.
- **M0 desktop + CI (2026-05-08).** Cargo workspace + per-PR & pre-release CI on Windows. Mobile portion still in-progress (below).
- **M1 wire protocol & crypto (2026-05-08).** `docs/PROTOCOL.md` v1 spec; `core::protocol` + `core::crypto`; 64 tests; MSRV bumped 1.85 → 1.88 (RUSTSEC-2026-0009). Pushed to origin/main as commits `599870d` + `c7dcc0d` (post-review pairing fixes).
