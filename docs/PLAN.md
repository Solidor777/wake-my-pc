...
- **One PC, many owner devices.** A single daemon install pairs with multiple mobile devices (phone + tablet + ...). Keystore holds `Vec<PairingRecord>`, one per device. PinSet rebuilt per accept from all non-revoked pairings. Re-pairing the same SPKI replaces the prior record (typical: phone wipe + re-pair). No artificial limit on count; revocation is per-device. Shared / multi-USER authority (different humans on one PC) is M9 — that adds an authorization model on top of this baseline authentication.
...
- **M0 desktop + CI (2026-05-08).** Cargo workspace (`core`, `daemon`, `desktop-ui`, `bindings`), edition 2024, MSRV 1.85 (bumped to 1.88 in M1). Workspace lints enforce Principle 2 (`unwrap_used`/`expect_used` denied; `forbid(unsafe_code)` in `core` + `bindings`). `core::hello()` smoke test wired through daemon, desktop-ui (egui), bindings (uniffi). Local `cargo build/test/clippy/fmt` green on Windows. CI workflows: `per-pr.yml` (Linux/Mac/Windows desktop + cargo-audit) and `pre-release.yml` (full five-platform; mobile gated to `mobile-ci` label / release tags). README.md.
- **M1 wire protocol & crypto (2026-05-08).** `docs/PROTOCOL.md` (versioned envelope + 8 ClientFrame / 5 DaemonFrame variants + pairing handshake §8 + KAT lock §10). `core::protocol` (postcard-encoded enums, length-prefixed envelope, defensive decoder). `core::crypto` (rcgen-generated Ed25519 device cert, SPKI-pinning rustls verifiers for both directions, TLS-1.3-only config builders with resumption disabled, pairing handshake state machine, strict-monotonic nonce counter). 59 tests passing across KAT (16) + property/negative (15) + crypto units (24) + real in-memory TLS handshake (3 — pinned/imposter-client/imposter-server). MSRV bumped 1.85 → 1.88 to absorb RUSTSEC-2026-0009 fix. `cargo audit` clean (exit 0; 3 unmaintained-but-not-vulnerable warnings tracked in TODO.md). All four per-PR CI gates green locally on Windows.
...
- **M2 Windows baseline (2026-05-08).** Daemon binary implements LAN listener (tokio + tokio-rustls 0.26 + rustls 0.23 with ring provider, TLS 1.3 only, resumption disabled). Per-connection protocol loop with strict-monotonic nonce validation, peer-SPKI → pairing-record mapping, dispatch to OS handlers. Windows OS handlers: `SetSuspendState`, `LockWorkStation`, `ExitWindowsEx(EWX_POWEROFF | EWX_FORCE)` with `SE_SHUTDOWN_NAME` privilege enabled at startup. Coarse session-state probe via `WTSGetActiveConsoleSessionId`. Encrypted keystore via DPAPI (`CryptProtectData`/`CryptUnprotectData`) with atomic-rename writes; postcard-encoded plaintext layer. Re-auth state machine + global `pc_reauth_interval_days`. CLI subcommands `serve`, `pair`, `list-paired`, `revoke <name>`, `reauth-now`. **Multi-device pairing supported** by design (`Vec<PairingRecord>`, PinSet rebuilt per accept; same-SPKI re-pair replaces prior record). 7 daemon integration tests + 5 re-auth unit tests; 75 total tests across workspace; clippy/fmt/audit clean. **Deferred within M2** to follow-up sessions: macOS + Linux platform handlers + keystore (Keychain / libsecret); mDNS/Bonjour service advertisement; service installation (SCM / launchd / systemd); uninstaller scripts; re-auth credential prompts (Windows Hello WebAuthn / Touch ID / polkit) — currently always-ok stub; re-auth notifications; lock-vs-unlocked discrimination (`WTSSessionInfoEx`); revoke retry queue; mockable `Handlers` trait for destructive-command integration tests.
